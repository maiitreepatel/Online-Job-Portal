...
if (isset($_GET['title']) && ($_GET['category'])) {
	$cate = $_GET['category'];
	$title = $_GET['title'];
	$query1 = "SELECT * FROM tbl_jobs WHERE category = :cate AND title = :title ORDER BY enc_id DESC LIMIT $page1,12";
	$query2 = "SELECT * FROM tbl_jobs WHERE category = :cate AND title = :title ORDER BY enc_id DESC";
	$fromsearch = true;

	$slc_title = "$title";
	$slc_category = "$cate";
	$title = "$slc_title Jobs in  $slc_category";
} else {
								...
								<div class="result-list-wrapper">
									...
									try {
										...
										if ($fromsearch == true) {
											...
											$stmt->bindParam(':title', $slc_title);
										}
										$stmt->execute();
										$result = $stmt->fetchAll();
										foreach ($result as $row) {
