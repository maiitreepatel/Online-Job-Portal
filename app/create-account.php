...
function register_as_employee()
{
    try {
        ...
        // Echo JavaScript code to display alert message after successful registration
        echo "<script>alert('Account created successfully as Employee!');</script>";

        // Redirect to registration page after a short delay
        echo "<script>setTimeout(function() { window.location.href = '../register.php?p=Employee&r=1123'; });</script>";
    } catch (PDOException $e) {
...
function register_as_employer()
{
	...
	VALUES (:fname, :title, :email, :lastlogin, :login, :role, :memberno)");
        ...
        $stmt->execute();
        echo "<script>alert('Account created successfully as Employer!!');</script>";
